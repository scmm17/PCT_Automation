...
var emailers = [
  {
    ...
    {
    name: "Margie",
    email: "margaret.wood@comcast.net"
  }
];

var familiesPerEmail = 8;
...
var preamble = '<br><br>Here is the list of families to contact for this week\'s Pastoral Care Team email notifications. Click on "Email Whole Family" to create an email to the entire family, or single individual, and paste in the body of the email. Use the phone number to call if no email link is present<br><br>';
...
{
  setLastFamilyName('Jeff & Jenn Stone');
  //setLastFamilyName('');
}
...
function showLastEmailed()
{
  Browser.msgBox('Last Emailed: ' + getLastFamilyName());
}
...
function addFamilyToEmail(body, familyRange) {
  ...
  for(var i = index+1; i <= index + size; i++) {
    ...
    var isNonMember = data[i][statusIndex] === "Adult Child - Non Member";
    if (isNonMember)
      continue;                              
...
function sendEmail(emailer, body) {
  ...
  GmailApp.sendEmail(emailer.email, 'Weekly Pastoral Care Team emails', "", {htmlBody: body});
}
